...
# extracting characters in config file
a = [line[:-1].replace('  ', '') for line in z]
...
def match_host(list):
    ...
    for index, items in enumerate(list):
        ...
        # current_host['Port'] = list[index + 1] # uncomment here if port
        # needed
...
# we have a list of dictionaries that show the hosts
hosts_dicts_list = match_host(flatten_list)
x.close()
